...
export default function CatalogPage<T extends { handle: string }>({
  title,
  columns,
  query,
  search,
  ...
  return (
    <ListWrapper title={title} actions={actions}>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
                ...
                return (
                  ...
                  >
                    <span className="inline-flex items-center gap-1">
                      {col.label}
                      {col.sortField && (
                        <SortIcon active={sort === col.sortField} asc={dir === 'asc'} />
                      )}
                    </span>
                  </th>
                );
          ...
          <tbody>
            ...
            {isError && (
              <tr>
                <td colSpan={columns.length} className="py-10 text-center text-sm text-red-400">
            ...
            {items.map((item) => (
              ...
              >
                ...
                {rowActions && <td className="py-2.5 px-3 text-right">{rowActions(item)}</td>}
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {data && (
        <CatalogPagination page={page} total={data.total} limit={data.limit} onPage={onPage} />
      )}
    </ListWrapper>
  );
}
