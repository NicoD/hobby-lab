...
export function useCatalogPaints(criteria: Criteria = {}) {
    ...
    const searchCatalogPaints = useCallback(async (query: string): Promise<SearchResult[]> => {
        ...
        const data = await apiFetch<ListResponse<CatalogPaint>>(`/api/color-lab/catalog/paints?${params}`)
    ...
    const createCatalogPaint = useMutation({
        mutationFn: (paint: { name: string, brand: string|null, range: string|null, paintType: string|null, color: string|null }) => apiFetch<CatalogPaint>('/api/color-lab/catalog/paints', {
            method: 'POST',
